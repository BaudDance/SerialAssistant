...
<template>
  <input id="ascii-modal" type="checkbox" class="modal-toggle">
  <label for="ascii-modal" class="modal">
    <label class="modal-box w-11/12 max-w-5xl" for="">
      <h3 class="text-lg font-bold mb-8">ASCII码表</h3>
      <div class="w-full">
        <table class="table w-full">
          <thead>
            <tr>
              ...
              <th>十六进制</th>
              <th>
                字符<span class="text-orange-700">(橙色为可显示字符)</span>
              </th>
              <th>描述</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="item in ascii" :key="item.decimal">
              <td>{{ item.binary }}</td>
              <td>{{ item.decimal }}</td>
              <td>{{ item.hex }}</td>
              <td :class="item.visible ? 'text-orange-700' : ''">
                {{ item.char }}
              </td>
              <td>{{ item.desc }}</td>
            </tr>
